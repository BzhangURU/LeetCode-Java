public class Solution {
    public int[][] generateMatrix(int n) {
        ...
        int circle=0;
		int count=1;
		while(circle<=matrix.length-1-circle){
			...
			for(int line=circle+1;line<matrix.length-circle;line++){
				matrix[line][matrix[0].length-1-circle]=count;
				count++;
			...
			for(int col=matrix[0].length-2-circle;col>=circle;col--){
				matrix[matrix.length-1-circle][col]=count;
				count++;
